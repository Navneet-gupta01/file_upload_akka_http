...
trait BaseRouteDefination extends ApiResponseJsonProtocol {
  import BaseRouteDefination._
  ...
  def serviceAndComplete[T: ClassTag](msg: Any, ref: ActorRef)(implicit format: JsonFormat[T]): Route = {
    ...
    onComplete(fut) {
      ...
      case Success(Failure(FailureType.Validation, ErrorMessage.InvalidEntityId, _)) =>
        complete((NotFound, NotFoundResp))

      case Success(fail: Failure) =>
